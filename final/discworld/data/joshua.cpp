...
void makenontrivial(int cost, int maxdist)
{
    queue<p3> q;
    q.emplace(start.first, start.second, 1);
    q.emplace(goal.first, goal.second, 1);
...
void case1()
{
    ...
    magic = int(1e8);
    setup();
    makenontrivial(magic, 30);

    rep(i, n) rep(j, n) if (pfield[i][j]!=-1) pfield[i][j] += randint(200, 1000);
    rep(i, n) rep(j, n) if (mfield[i][j]!=-1) mfield[i][j] += randint(5, 10);
